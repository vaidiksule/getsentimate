...
export default function AnalysisForm({ onSubmit, isMutating, credits }: AnalysisFormProps) {
  ...
  const disabled = isMutating || submitting || (credits === null || credits < 1);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <form
        ...
        className="flex flex-col gap-4 rounded-3xl border border-neutral-200 bg-white/80 p-6 shadow-md backdrop-blur-sm"
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex-1">
            <Input
              ...
              className="h-12 rounded-xl border-neutral-200 bg-white px-4 text-sm text-black shadow-inner focus-visible:ring-2 focus-visible:ring-[#0A84FF] placeholder:text-neutral-400"
              autoComplete="off"
              {...form.register("url")}
              disabled={disabled}
            />
          ...
          <Button
            ...
            size="lg"
            disabled={disabled}
            className="h-12 px-8 rounded-xl bg-[#0A84FF] text-sm font-semibold text-white shadow-md transition hover:bg-[#0b7aed] focus-visible:ring-2 focus-visible:ring-[#0A84FF]/70 whitespace-nowrap"
          >
            {disabled ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                {credits === null ? 'Loading...' : hasInsufficientCredits ? 'No credits' : 'Analyzing…'}
              </>
            ) : (
              <>
        ...
        {/* Error message - separate from input/button row */}
